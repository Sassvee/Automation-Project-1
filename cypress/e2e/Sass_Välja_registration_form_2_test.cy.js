...
describe('Section 1: Functional tests', () => {
    ...
    it('User can use only same both first and validation passwords', ()=>{
        ...
        cy.get('#username').type('SassV')
        cy.get('#email').type('Sassvalja@gmail.com')
        cy.get('[data-cy="name"]').type('Sass')
        cy.get('#lastName').type('Välja')
        cy.get('[type="number"]').type('555666777')
        
        // Type confirmation password which is different from first password
        cy.get('#password').type('essvee')
        cy.get('#confirm').type('essvee1')
    ...
    it('User cannot submit form when some mandatory field is not present', ()=>{
        // Add at least 1 test for checking some mandatory field's absence
        inputValidData('SassV')

        // Clear a mandatory field "First Name"
...
describe('Section 2: Visual tests', () => {
    it('Check that logo is correct and has correct size', () => {
        ...
        cy.get('#logo').invoke('height').should('be.lessThan', 167).and('be.greaterThan', 165)
        cy.get('#logo').invoke('width').should('be.lessThan', 179).and('be.greaterThan', 177) 

    })

    it('My test for second picture', () => {
        // Test for checking that the Cypress logo is correct and has the correct size
        cy.log('Will check logo source and size')
